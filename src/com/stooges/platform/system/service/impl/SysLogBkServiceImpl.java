...
import com.stooges.platform.system.dao.SysLogBkDao;
import com.stooges.platform.system.service.SysLogBkService;
...
public class SysLogBkServiceImpl extends BaseServiceImpl implements SysLogBkService{
  ...
  public List findExportData(Map sysLogBk){
	...
	public String creatFile(List<Map<String, Object>> list) throws Exception {
        ...
        String enfileName = UUIDGenerator.getUUID() + ".stoogesbk" ;
		...
		PlatFileUtil.encrypt(realPath,
				enrealPath, PlatFileUtil.ENCRTPT_KEY);
	...
	public Map<String, Object> impExcelDatas(String excelFilePath, String type) {
		...
		String key = PlatFileUtil.readFileLastByte(excelFilePath, PlatFileUtil.ENCRTPT_KEY.length());
		if(PlatFileUtil.ENCRTPT_KEY.equals(key)){
			try{
				String trgUrl = excelFilePath.substring(0, excelFilePath.lastIndexOf("/"))
						+UUIDGenerator.getUUID()+".json";
				PlatFileUtil.decrypt(excelFilePath, trgUrl,  PlatFileUtil.ENCRTPT_KEY.length());
