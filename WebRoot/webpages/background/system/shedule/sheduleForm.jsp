...
<link href="webpages/background/system/shedule/css/easyui.min.css" rel="stylesheet"
	type="text/css" />
<link href="webpages/background/system/shedule/css/icon.css" rel="stylesheet" type="text/css" />
<style type="text/css">
.line {
	height: 25px;
	line-height: 25px;
...
<body>
				...
				<fieldset style="border-radius: 3px; height: 220px;">
					<table style="height: 100px;">
						<tbody>
							...
							<tbody id="SheduleFormDiv">
							<input  type="hidden" name="SHEDULE_ID" value="${shedule.SHEDULE_ID}"/>
							<tr>
								<td>Cron 表达式:</td>
								<td colspan="6"><input id="cron"  type="text" name="SHEDULE_CRON" 
									style="width: 100%;" class="form-control" value="${shedule.SHEDULE_CRON}" 
									/></td>
							...
							<tr>
								<td>定时器编码:</td>
								<td ><input type="text" name="SHEDULE_CODE" value="${shedule.SHEDULE_CODE}"
								    <c:if test="${shedule.SHEDULE_ID!=null}">readonly="readonly"</c:if>
									style="width: 100%;" class="form-control" data-rule="required;" /></td>
								<td>定时器名称:</td>
								<td><input type="text" name="SHEDULE_NAME" value="${shedule.SHEDULE_NAME}"
									style="width: 100%;" class="form-control" data-rule="required;" /></td>
								<td>绑定IP地址:</td>
								<td><input type="text" name="SHEDULE_BINDIP" value="${shedule.SHEDULE_BINDIP}"
									style="width: 100%;" class="form-control" data-rule="required;" /></td>
							</tr>
							<tr>
								<td>处理类路径:</td>
								<td colspan="6"><input  type="text" name="SHEDULE_CLASSNAME" data-rule="required;"
									style="width: 100%;" class="form-control" value="${shedule.SHEDULE_CLASSNAME}" 
									/></td>
							</tr>
							</tbody>
						</tbody>
				  ...
				  <button class="btn btn-outline btn-primary btn-sm" onclick="submitSheduleForm();" type="button" >提交</button>
	...
	<script src="webpages/background/system/shedule/js/jquery.min.js"
		type="text/javascript"></script>
	<script src="webpages/background/system/shedule/js/jquery.easyui.min.js" type="text/javascript"></script>
	<script type="text/javascript">
	  $(function(){
		  btnFan();
	  });
	  function submitSheduleForm(){
	    var formData = PlatUtil.getFormEleData("SheduleFormDiv");
		PlatUtil.ajaxProgress({
			url:"system/SheduleController.do?saveOrUpdate",
			params : formData,
