...
var PlatUtil = {
		...
		if(showProgress){
			if(parent.layer){
				msgIndex = parent.layer.msg(progressMsg, {
					  icon: 16,
					  shade: 0.01,
					  time:config.timeout?config.timeout:30000
				 });
			}else{
	...
	getBackPlatLoginUser:function(){
		var userinfo = parent.$("#__BACKPLATUSERJSON").val();
		if(!userinfo){
			userinfo = $("#__BACKPLATUSERJSON").val();
		}
		if(!userinfo){
			userinfo = top.parent.$("#__BACKPLATUSERJSON").val();
		}
		if(userinfo){
